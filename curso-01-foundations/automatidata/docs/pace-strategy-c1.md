...
Um detalhe do mín./máx. merece destaque porque sustenta a leitura dos estornos: o piso negativo não está só na tarifa. `extra` chega a -$1,00, `mta_tax` a -$0,50 e `improvement_surcharge` a -$0,30, o que mostra que a reversão alcança também as taxas, e não apenas o valor principal. São mínimos por coluna, porém, e não provam uniformidade: a conferência linha a linha encontrou exceções pontuais em `mta_tax`. Cada item das duas últimas categorias vira um filtro explícito que conta quantos registros existem, em vez de ficar na leitura do extremo isolado.
...
**5. A corrida de $1.200,29.** A causa é indeterminada, e é isso que a torna uma pergunta para a TLC: a tarifa negociada (`RatecodeID` 5) torna um valor alto tecnicamente admissível, enquanto a desproporção com 2,60 milhas o torna improvável — daí a classificação de provável erro de lançamento, a validar. Está em último lugar porque é caso isolado: o segundo maior total do arquivo é $450,30, e incluir ou remover um único registro não altera o modelo de forma material. Vale perguntar; não vale esperar a resposta para seguir.
...
**Possíveis, mas suspeitas.** A corrida de $1.200,29 — tarifa de $999,99 mais $200,00 de gorjeta em 2,60 milhas, sob tarifa negociada: provável erro de lançamento, a validar com a TLC, sem que a inspeção permita afirmá-lo, porque a tarifa negociada mantém o valor tecnicamente admissível; 46 corridas acima de três horas, com máximo de 1.439,55 minutos (praticamente 24 horas); e a concentração de valores altos em corridas com `RatecodeID` 5 e destino na zona 265, não identificada. Nenhuma viola uma regra física, todas destoam do padrão.

**Comportamento do sistema, que se confunde com anomalia.** Os 14 valores negativos não são erro: são estornos coerentes, com `payment_type` 3 ou 4 e reversão consistente na maioria dos campos, com exceções pontuais em `mta_tax`. E a gorjeta de $0,00 exatos nas 7.267 corridas em dinheiro não é ausência de gorjeta, é ausência de captura — a anomalia aqui está no instrumento, não no dado.
...
O item que classifiquei como mais crítico entra aqui por uma porta parecida: a representatividade da amostra não exige dado novo, exige documentação — o critério de extração dos 22.699 registros. Na falta dele, bastaria a contagem de corridas por mês e por vendor na base completa de 408.294, o suficiente para comparar a distribuição da amostra com a do universo e estimar o viés antes de qualquer modelagem.
